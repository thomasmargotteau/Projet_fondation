...
nomImages = ("Images\PhotoCam2_1.jpg", "Images\ConfigBlocsAvecArucoCote1.jpg", "Images\PhotoCarteTelephone5.jpg",
...
while True:
    ...
    if ids is not None:
        ...
        # Open the image
        image = Image.open("Images\Transformed_image.jpg")  # Replace "frame.jpg" with the path to your image file
        ...
        cropped_image.save("Images\cropped_frame.jpg")
        cropped_image.show()
        # Display the result
        ...
        for i in range(5):
            if i == 0 :
                #rouge1
                bound_lower = np.array([0, 130, 100])
                bound_upper = np.array([10, 255, 255])
                mask1 = cv2.inRange(hsv_img, bound_lower, bound_upper)
            elif i == 1 :
                # rouge2
                bound_lower = np.array([170, 130, 100])
                bound_upper = np.array([180, 255, 255])
                mask2 = cv2.inRange(hsv_img, bound_lower, bound_upper)
            elif i == 2 :
                bound_lower = np.array([0, 0, 200])
                bound_upper = np.array([255, 15, 255])
                mask3 = cv2.inRange(hsv_img, bound_lower, bound_upper)
        ...
        display_mask1 = cv2.bitwise_and(dst, dst, mask=mask1)
        ...
        display_mask3 = cv2.bitwise_and(dst, dst, mask=mask3)

        cv2.imshow("Result 1", display_mask1)
        cv2.imshow("Result 2", display_mask2)
        cv2.imshow("Result 3", display_mask3)
